...
const BidsPieStat3 = (props) => {
              ...
              return (
                <div key={`customLegend${index}`}>
                  ...
                  <div className="cta-3 text-primary value">
                    {chart.data.datasets[0].data[0].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} / {(chart.data.datasets[0].data[1] + chart.data.datasets[0].data[0]).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                  </div>
                </div>
              );
            })}
  ...
  const data = React.useMemo(() => {
    return {
      labels: ['Bid Win over Total Bids at Inspecting Price'],
      datasets: [
        {
          data: props.data,
...
export default React.memo(BidsPieStat3);
