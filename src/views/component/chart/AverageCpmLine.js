...
const AverageCpmLine = (props) => {
  /*-------------*/
  const { themeValues } = useSelector((state) => state.settings);
  const chartContainer = useRef(null);
  const tooltipRef = useRef(null);
  const SelectInspectedPricePlugin = React.useMemo(() => {
    return {
      id: 'selectLastOne',
      afterDraw: (chart) => {
        const { tooltip } = chart;
        if (tooltip.getActiveElements().length === 0) {
          ...
          tooltip.setActiveElements(
            [
              {
                datasetIndex: 0,
                index: props.data.map((el) => el.Range).indexOf(props.info.Floor),
              },
            ],
            {
              x: rect.left + point.x,
  ...
  const data = React.useMemo(() => {
    return {
      labels: props.data.map((el) => el.Range),
      datasets: [
        {
          label: 'Highest Average CPM over Bid Ranges',
          data: props.data.map((el) => {
            return (el.CPM[el.CPM.indexOf(Math.max(...el.CPM))]);
          }),
  ...
  const config = React.useMemo(() => {
    return {
      type: 'line',
      plugins: [SelectInspectedPricePlugin],
      options: {
        layout: {
          padding: 0,
        },
  ...
  }, [data, SelectInspectedPricePlugin, ExternalTooltip]);
  ...
  return (
    <Row className="g-0 d-flex w-100 align-items-center">
      ...
      <Col xs="auto">
        <canvas ref={chartContainer} className="sw-80 sw-xl-100 sh-10" />
      </Col>
    </Row>
  );
};

export default React.memo(AverageCpmLine);
